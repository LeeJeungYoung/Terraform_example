resource "aws_instance" "aws03_jenkins_server" {
  ami                    = var.ami_id
  ...
  iam_instance_profile = data.aws_iam_instance_profile.aws03_ec2_profile.name
...
resource "aws_lb_target_group_attachment" "aws03_jenkins_attach" {
  target_group_arn = data.aws_lb_target_group.aws03_jenkins_tg.arn
  target_id        = aws_instance.aws03_jenkins_server.id
  port             = 80
}