...
data "aws-security_group" "aws03_http_sg" {
  filter {
    name   = "tag:Name"
...
data "aws_iam_instance_profile" "aws03_ec2_instance_profile" {
  name = "${var.prefix}-ec2-instance-profile"
}

data "aws_ami" "aws03_instance_ami" {
  most_recent = true
  owners     = ["self"]
...
data "aws_lb_target_group" "aws03_alb_was_group" {
