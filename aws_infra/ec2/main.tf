...
resource "aws_instance" "aws03_instance" {
  ami           = var.ami_id
  instance_type = var.instance_type
  associate_public_ip_address = true
  key_name      = var.key_name
  subnet_id     = data.aws_subnet.aws03_public_subnet.id
...
#codedeply agent, docker 설치 대기
resource "null_resource" "aws03_delay" {
  provisioner "local-exec" {
    command = "sleep 180"
  }
  depends_on = [aws_instance.aws03_instance]
  
}

#원본 instance 를 이용해 ami 생성
